    ...
    generateB2bOrdersSheet: async ({
        ...
        try {
            ...
            if (bookingType === "ticket") {
                ...
                ws.cell(1, 15).string("Booking Confirmation No").style(titleStyle);
            }
            ...
            for (let i = 0; i < orders.length; i++) {
                ...
                ws.cell(i + 2, 4).string(order?.createdAt ? formatDate(order?.createdAt) : "N/A");
                ...
                ws.cell(i + 2, 10).number(Number(order?.activities?.adultsCount) || 0);
                ws.cell(i + 2, 11).number(Number(order?.activities?.childrenCount) || 0);
                ws.cell(i + 2, 12).number(Number(order?.activities?.infantCount) || 0);
                ws.cell(i + 2, 13).string(order?.activities?.transferType || "N/A");
                ws.cell(i + 2, 14).string(order?.activities?.driver?.name || "N/A");
                if (bookingType === "ticket") {
                    ...
                    ws.cell(i + 2, 15).string(JSON.stringify(allTickets) || "N/A");
                } else {
                    ws.cell(i + 2, 15).string(
                        order?.activities?.bookingConfirmationNumber || "N/A"
                    );
                }
                ws.cell(i + 2, 16).number(Number(order?.activities?.grandTotal) || 0);
                if (downloader === "admin") {
                    ws.cell(i + 2, 17).number(Number(order?.activities?.profit) || 0);
                } else if (downloader === "reseller") {
                    ws.cell(i + 2, 17).number(Number(order?.activities?.resellerMarkup) || 0);
                } else if (downloader === "sub-agent") {
                    ws.cell(i + 2, 17).number(Number(order?.activities?.subAgentMarkup) || 0);
                } else {
