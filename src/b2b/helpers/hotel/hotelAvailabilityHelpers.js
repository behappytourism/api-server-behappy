...
const { applyPromotion, getPromotionsWithCancellationType } = require("./hotelPromotionHelpers");
    ...
    try {
        ...
        for (let i = 0; i < contracts.length; i++) {
            ...
            if (contract?.contractGroup?.isDeleted === false) {
                ...
                console.log("ROOM RATES", rooomRates?.length);
                        ...
                        if (boardTypesWithKeyVal[contract?.basePlan]) {
                            if (objIndex === -1) {
                                roomPerDay?.rooms.push({
                                    ...
                                    code: selectedRoomOccupancies[0]?.shortName,
                                    provider: "Contract",
                                    ...
                                    roomType: {
                                        ...
                                        descripttion: roomType?.descripttion,
                                    },
                                    rates: [
                                        {
                                            provider: "Contract",
                                            detailsKey: `${hotelProviderId}|${hotel?.hbId}|${roomType?._id}`,
                        ...
                        if (
                            ...
                            for (let ab = 0; ab < mealSupplements.length; ab++) {
                                if (boardTypesWithKeyVal[mealSupplements[ab]?.boardType]) {
                                    ...
                                    if (mealSupplements[ab]?.childPrice > 0) {
                                        mealSupplementPrice += mealSupplements[ab]?.childPrice;
                                    }
                ...
                for (let k = 0; k < promiseRes.rooms.length; k++) {
                    if (
                        ...
                        for (let l = 0; l < obj?.rates?.length; l++) {
                            ...
                            let isSameContract = promiseRes?.rooms[k]?.rates?.findIndex(
                                (rateObj) => {
                                    return (
                                        rateObj?.contract?.toString() ===
                            ...
                            for (let m = 0; m < promiseRes?.rooms[k]?.rates?.length; m++) {
                                ...
                                if (
                                    ...
                                    (isSameContract !== -1
                                        ? myRate?.contract?.toString() ===
                                          promiseRes?.rooms[k]?.rates[m]?.contract?.toString()
                                        : true)
                                ) {
...
const addPromotionOnHotelPricesSG = async ({
    ...
    try {
        ...
        for (let i = 0; i < availabilityData.length; i++) {
            ...
            if (profileMarkup) {
                for (let mi = 0; mi < profileMarkup?.hotel?.length; mi++) {
                    if (profileMarkup?.hotel[mi]?.hotelId?.toString() === hotelId?.toString()) {
            ...
            for (let j = 0; j < availabilityData[i].rates?.length; j++) {
                ...
                const contractCancellationPolicies = cancellationPolicies;
                ...
                for (let l = 0; l < contractGroups.length; l++) {
                    ...
                    if (availabilityData[i].rates[j].promotions) {
                        ...
                        availabilityData[i].rates[j].appliedPromotions = appliedPromotions;
                    }
                    availabilityData[i].rates[j].appliedPromotions = appliedPromotions2;
                ...
                availabilityData[i].rates[j].grossPrice +=
                ...
                availabilityData[i].rates[j].rateKey += `${appliedPromotionId}|`;
                ...
                availabilityData[i].rates[j].appliedPromotionCancellation =
                    appliedPromotionCancellation;
                availabilityData[i].rates[j].rateKeyRaw = availabilityData[i].rates[j].rateKey;

                if (needHashedRate) {
                    const hashedRateKey = md5(availabilityData[i].rates[j].rateKey);
                    availabilityData[i].rates[j].rateKey = hashedRateKey;
                }
                ...
                if (
                    ...
                    if (cancellationType) {
                        ...
                        for (const type of cancellationTypes) {
                            const foundType = promotionsWithCancellationType[type][0];
                            ...
                            if (foundType) {
                                const {
                                    ...
                                    promotions: [foundType],
                                    selectedContract: contractGroups[contractGroupIndex],
                                    roomType,
                                    bookBefore,
                                    basePlan,
                                    roomTypesWithKeyVal,
                                    boardTypesWithKeyVal,
                                    isFirstIteration: contractGroupIndex === 0,
                                });
                                ...
                                if (
                                    ...
                                    if (foundType?._id === promotion?._id) {
                                        const parentMarkup =
                                            ...
                                            saMarkup +
                                            clMarkup;
                                        const clonedRate = await createCloneRate(
                                            totalOffer,
                                            foundType,
                                            appliedPromotions2,
                                            availabilityData[i].rates[j],
                                            basePlan,
                                            parentMarkup,
                                            marketMarkup,
                                            b2bMarkup,
        ...
        async function createCloneRate(
            totalOffer,
            foundRefundable,
            promotion,
            rate,
            ...
            clonedRate.cancellationPolicies = foundRefundable?.cancellationPoliciesTxt;
            ...
            let newRateKey = clonedRate.rateKeyRaw.split("|"); // Step 1: Split the string by "|"
            newRateKey[10] = `${foundRefundable?._id}`;
            clonedRate.rateKey = newRateKey.join("|");
            clonedRate.isClonedRate = true;
            clonedRate.rateKeyRaw = clonedRate.rateKey;
            if (needHashedRate) {
                const hashedRateKey = md5(clonedRate.rateKey);
                clonedRate.rateKey = hashedRateKey;
            }
