...
const getSingleContractPriceORD = async ({
    ...
    try {
        ...
        if (!roomRates || roomRates?.length < 1) {
                    ...
                    for (let l = 0; l < combinations?.length; l++) {
                        ...
                        if (noOfInfants > 0 || noOfChildren > 0 || extraBedAdults > 0) {
                            ...
                            roomRates?.forEach((rmRate) => {
                                if (
                                    rmRate?.roomTypes[roomTypeObjIndex]?.roomOccupancies[
                                        rateOccupancyIndex
                                    ]?.price &&
                                    (!tempRoomPrice ||
                                        rmRate?.roomTypes[roomTypeObjIndex]?.roomOccupancies[
        ...
        if (extraMealSupplement) {
            ...
            if (mealSupplement?.childPrice) {
                contractRate.mealSupplementPrice += mealSupplement?.childPrice;
            }
...
const applyPromotionOnORD = async ({
    ...
    try {
        ...
        for (let l = 0; l < contractGroups.length; l++) {
            ...
            let promotions = await HotelPromotion.find({
                hotel,
            ...
            if (promotionId) {
                const promotion = promotions?.find((prom) => prom?._id?.toString() === promotionId);
            ...
            if (contractGroups[l].dates?.some((item) => item?.date === fromDate) && promotion) {
