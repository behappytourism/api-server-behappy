...
const b2bOrderSchema = Joi.object({
    ...
    isPromoCodeApplied: Joi.boolean().required(),
    promoCode: Joi.string().when("isPromoCodeApplied", {
        is: Joi.boolean().valid(true),
        then: Joi.string().required(),
    }),
});

const b2bOrderCancellationSchema = Joi.object({
    attractionCancellations: Joi.array().items({
        id: Joi.string().required(),
        charge: Joi.number().required(),
        cancellationId: Joi.string().allow("", null),
    }),
    transferCancellations: Joi.array().items({
        id: Joi.string().required(),
        charge: Joi.number().required(),
        cancellationId: Joi.string().allow("", null),
    }),
    cancellationRemark: Joi.string().required(),
});
module.exports = { b2bOrderSchema, b2bOrderCancellationSchema };
