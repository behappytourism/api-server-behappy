...
const B2BBanner = require("../../../models/global/b2bBanner.model");
const { B2BHomeSetting } = require("../../models");

module.exports = {
    getHomeBannaers: async (req, res) => {
        try {
            const banner = await B2BBanner.findOne({ name: "home", isDeleted: false });
    ...
    getHomeSections: async (req, res) => {
        try {
            const homeSections = await B2BHomeSection.aggregate([
                {
                    $match: {
                        isDeleted: false,
                    },
    ...
    getContactDetails: async (req, res) => {
        try {
            ...
            res.status(200).json({
                ...
                addresses: addresses || [],
                footer:
                    home?.footer.map((footer) => {
                        const sortedNavLinks = [...footer?.navLinks].sort(
                            (a, b) => (a?.displayOrder || 0) - (b?.displayOrder || 0)
                        );
                        return { ...footer, navLinks: sortedNavLinks };
                    }) || [],
    ...
    getAboutUs: async (req, res) => {
        try {
            ...
            if (!home) {
                return sendErrorResponse(res, 404, "About us  details not added");
            }
